...
public class ContratosBean implements Serializable {
	...
	/**
	 * @param cab_observacion
	 *            the cab_observacion to set
	 */
	...
	public String cargarDatosContrato(ARR_Contratos_Cab c) throws Exception {
		...
		if (c.getCab_estado() == 'F') {
			...
			for (ARR_ContratoArticulos a : cd) {
				if (a.getCa_estado() == 'A'
						&& a.getCon_det().getCon_cab().getCab_numero()
								.equals(c.getCab_numero())) {
					list2.add(a);
					if (list.size() == 0) {
						list.add(a.con_det);
					}
			...
			if (y == cd.size()) {
				list.add(sitioObtener(c.getCab_numero()));
			}
	...
	public void imprimirRptContrato() {
		...
		try {
			...
			// parametros.put("pApellidos",
			// c.getGenPersona().getPerApellidos());
	...
	public void mostrarRptContrato() {
		...
		try {
			...
			parametros.put("pFechaInicio",
					formateador.format(getCab_fechaini()));
			parametros
					.put("pFechaFinal", formateador.format(getCab_fechafin()));
			parametros.put("pRutaImage", carpetaReportes + File.separatorChar
					+ "yachay-logo1.png");
	 ...
	 * @return
	 */
	public ARR_Contratos_Det sitioObtener(String cc) {
		ARR_Contratos_Det p = new ARR_Contratos_Det();
		List<ARR_Contratos_Det> l = mngCont.findAllContratos_Det();
		for (ARR_Contratos_Det cd : l) {
			if (cc == cd.getCon_cab().getCab_numero()) {
				p = cd;
			}
		}
		return p;
	...
	/**
	 * Metodo para listar los articulos correcpondientes a un sitio de un
	 * contrato activado
	 * 
	 * @return
	 */
